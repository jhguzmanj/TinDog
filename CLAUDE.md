...
## Canciones nuevas: pasan por el lector antes de entrar en `SONGS`
`tools/lector-partituras/` es una app aparte (también un HTML que se abre sin servidor)
para transcribir una partitura, **escucharla** y corregirla antes de que llegue aquí.
Exporta el objeto `{id, cat, name, tip, tempo, steps}` ya con la forma de `SONGS`, con
